...
describe Track do
  ...
  it 'selects all possible arrays of talks with given 180 minutes' do
    possibles = Track.possibleTalksOfGivenSum(@conference.talks, 180)
  ...
  it 'should be able to assign some talks to two session of one track' do
    talks1 = Track.possibleTalksOfGivenSum(@conference.talks,180).first
    talks2 = Track.possibleTalksOfGivenSum(@conference.talks,240).first
  ...
  it 'should be able to delete talks from the entire talks' do
    talks1 = Track.possibleTalksOfGivenSum(@conference.talks,180).first
    track.deleteSessionTalks(talks1, @conference.talks)
    talks1.each do |talk|
      expect(@conference.talks.include?(talk)).to eql(false)
    end
  end
